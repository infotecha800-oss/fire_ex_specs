...
function initGridAnimation() {
  ...
  function draw() {
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols - 1; c++) {
        ...
        if (total > 0.01) {
          ...
          g.addColorStop(0.5, `rgba(21, 128, 61,${a + 0.1})`);
          g.addColorStop(1,   `rgba(21, 128, 61,${a * 0.8})`);
          ctx.strokeStyle = g;
          ctx.lineWidth = 0.5 + total * 1.2;
        } else {
          ctx.strokeStyle = 'rgba(21, 128, 61,0.045)';
          ctx.lineWidth = 0.5;
        }
    ...
    for (let c = 0; c < cols; c++) {
      for (let r = 0; r < rows; r++) {
        ...
        if (total < 0.01) {
          ...
          ctx.fillStyle = 'rgba(21, 128, 61,0.10)'; ctx.fill();
        } else {
          const dotR = 1.8 + total * 5;
          const grd = ctx.createRadialGradient(x, y, 0, x, y, dotR);
          grd.addColorStop(0, `rgba(21, 128, 61,${0.9 * total})`);
          grd.addColorStop(1, 'rgba(21, 128, 61,0)');
          ctx.beginPath(); ctx.arc(x, y, dotR, 0, Math.PI * 2);
          ctx.fillStyle = grd; ctx.fill();
 
          if (total > 0.5) {
            ctx.beginPath(); ctx.arc(x, y, 1.8 + total * 2.5, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(21, 128, 61,${total * 0.9})`; ctx.fill();
          }
        }
      }
    }
    ...
    if (mx > 0 && mx < W && my > 0 && my < H) {
      ...
      rg.addColorStop(0, 'rgba(21, 128, 61,0.06)');
      rg.addColorStop(0.4, 'rgba(21, 128, 61,0.03)');
      rg.addColorStop(1, 'rgba(21, 128, 61,0)');
      ctx.fillStyle = rg;
    ...
    for (const cl of clicks) {
      ...
      rg2.addColorStop(0, 'rgba(21, 128, 61,0)');
      rg2.addColorStop(0.5, `rgba(21, 128, 61,${cl.life * 0.14})`);
      rg2.addColorStop(1, 'rgba(21, 128, 61,0)');
      ctx.fillStyle = rg2;
